...
func test_buying_installs_the_part_and_pays_for_it() -> void:
	...
	assert_bool(_run.buy(slot, Vector2i(1, 0))).is_true() # (1, 0)-(1, 2)
	assert_int(_run.gold).is_equal(6)
	assert_bool(_run.slots[slot].sold).is_true()
	var installed := _run.grid.get_part_at(Vector2i(1, 2))
...
func test_a_failed_buy_changes_nothing() -> void:
	...
	assert_bool(_run.buy(gatling_slot, Vector2i(0, 1))).is_false() # its tail hits the (0, 3) corner
	assert_int(_run.gold).is_equal(10)
	_run.gold = 3
	assert_bool(_run.buy(gatling_slot, Vector2i(1, 0))).is_false() # costs 4
	assert_bool(_run.buy(99, Vector2i(1, 0))).is_false()            # no such slot
...
func test_buying_uses_the_slot_rotation() -> void:
	...
	# On its side it fits row 1 from (0, 1), where upright it would hit a corner.
	assert_bool(_run.buy(slot, Vector2i(0, 1))).is_true()
...
func test_selling_refunds_in_full_this_round_and_half_later() -> void:
	assert_bool(_run.buy(_slot_of(_gatling), Vector2i(1, 0))).is_true() # 10 -> 6
	...
	assert_int(_run.sell(Vector2i(1, 1))).is_equal(4)                   # 3 -> 7
...
func test_moving_and_rotating_installed_parts_is_free() -> void:
	assert_bool(_run.buy(_slot_of(_gatling), Vector2i(1, 0))).is_true() # 10 -> 6
	assert_bool(_run.move(Vector2i(1, 0), Vector2i(1, 1))).is_true()     # (1, 1)-(1, 3)
	assert_bool(_run.rotate_placed(Vector2i(1, 3))).is_true()            # (1, 1)-(3, 1)
	assert_object(_run.grid.get_part_at(Vector2i(3, 1))).is_not_null()
	assert_bool(_run.move(Vector2i(2, 1), Vector2i(2, 1))).is_false()    # would run off to (4, 1)
	assert_int(_run.gold).is_equal(6)
	assert_bool(_run.is_fresh(Vector2i(3, 1))).is_true() # still sells for its full cost
	assert_int(_changes).is_equal(3) # the buy, the move, and the turn


func test_preview_buy() -> void:
	var slot := _slot_of(_gatling)
	var preview := _run.preview_buy(slot, Vector2i(1, 0))
	assert_int(preview.fit).is_equal(MechGridData.Fit.OK)
	assert_bool(preview.affordable).is_true()
	assert_array(preview.cells).contains_exactly_in_any_order(Vector2i(1, 0), Vector2i(1, 1), Vector2i(1, 2))
	# Everything around it except the (0, 0) corner and off the top edge.
	assert_array(preview.open_edges).has_size(6).contains_exactly_in_any_order(
		Vector2i(2, 0), Vector2i(0, 1), Vector2i(2, 1), Vector2i(0, 2), Vector2i(2, 2), Vector2i(1, 3))
	...
	var blocked := _run.preview_buy(slot, Vector2i(0, 1))
	assert_int(blocked.fit).is_equal(MechGridData.Fit.DISABLED_CELL)
	...
	_run.gold = 1
	var pricey := _run.preview_buy(slot, Vector2i(1, 0))
	...
	assert_object(_run.preview_buy(99, Vector2i(1, 0))).is_null()
...
func test_preview_move() -> void:
	...
	# Its own cell counts as free; off the grid doesn't fit.
	assert_int(_run.preview_move(Vector2i(0, 2), Vector2i(0, 2)).fit).is_equal(MechGridData.Fit.OK)
	assert_int(_run.preview_move(Vector2i(0, 2), Vector2i(4, 2)).fit).is_equal(MechGridData.Fit.OUT_OF_BOUNDS)
...
func _new_run(rng_seed: int) -> RunState:
	...
	rng.seed = rng_seed
	return RunState.new(Fixtures.cross_chassis(), [_gatling, _laser, _reactor, _heatsink],
