...
- **The result:** 0.8 seconds after the end (the one-shot `ResultTimer`), the `ResultPanel` comes up on its own layer: `result_title()` (VICTORY, DEFEAT, or DRAW, from the player's side), `record_line()` ("ROUND 3 · WINS 3 · LOSSES 1", this fight counted, draws once there are any), and `result_rows()` (battle duration, the player's total damage dealt, and the MVP weapon with its damage, or "—"). Its RETURN TO SHOP button emits `finished(winner)`, null for a draw.
...
- `FighterView`: a mech's chassis sprite at 2× with each mounted weapon's sprite at its bay's `battle_anchor`, behind the body (shaded) when `battle_behind`. The right side's is mirrored. It bobs smoothly, greys out while shut down, and goes dark once destroyed, through the `mech_sprite` shader. `get_muzzle(weapon)` and `get_center()` give where shots leave and land.
...
- `StormTimer`: whole seconds until the storm, then STORM. `RoundBadge` and `ResultPanel` are above.
