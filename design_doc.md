...
- **The Chassis (The Board):** The grid the player builds their mech on. A run starts by picking one of three frames, each with its own slot layout, base stats, and a passive that works in every fight:
  ...
  |---|---|---|---|---|---|---|
  | The Bastion | Tank / Attrition | Wide: 4 across, 3 down, all 12 slots | Back, over (1, 0) and (2, 0) | 45 | 2 | **Thick Plating:** every hit it takes is 1 smaller, never below 0. Weapons, meltdowns, and the storm alike. |
  | The Striker | Glass Cannon / Burst | Tall: 2 across, 5 down, all 10 slots | Left Arm and Right Arm beside rows 1-3, Back over row 0 | 22 | 4 | **Overclock:** the first weapon to fire in a fight fires twice. The second shot is free. |
  | The Reactor | Synergy / Combo | Diamond: the 13 cells of a 5×5 within 2 steps of the center, so the center touches 4 slots | Left Arm and Right Arm beside rows 1-3, each touching only a tip, (0, 2) or (4, 2) | 30 | 3 | **Meltdown:** at 100 heat it deals 25 damage to the enemy, cools to 0, and shuts down for 3 seconds: no part acts and the chassis adds no energy. |
...
- **The dummy** (`DUMMY`, built by `make_dummy(rules)`) is the opponent until there are real ones: a missile pod in the back bay of its own Bastion (`DUMMY_CHASSIS`), cooled by a heatsink touching the bay, whatever frame the player picked.
