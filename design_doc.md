...
- **Node kinds:** Battle (a normal enemy), Elite (a tougher enemy), Scrap Shop (buy and sell), Hangar (repair or reinforce), Event (a choice), Unknown (`~`: on arrival it turns into an event 50%, a battle 25%, a Salvage Cache 15%, or a shop 10%, on the run's "unknown" stream), Salvage Cache (`C`: free gold from the sector's battle range, a relic rolled like an elite's, and a field kit, on the loot screen titled SALVAGE CACHE), and the Boss. The bottom floor is all battles and the floor under the boss all hangars; elites and hangars don't appear before floor 5; an Elite, Hangar, or Shop never follows another of its kind on a path; elsewhere kinds are weighted (Battle 45, Event 22, Elite 10, Hangar 12, Shop 6, Unknown and Cache 0 by default; the sectors set Event 18, Unknown 6 / 8 / 8, and Cache 3 / 4 / 4; Sector 1 has Shop 8, Sector 3 Battle 43 and Elite 12; a Cache never follows a Cache), and the nodes a fork leads to differ where they can.
...
- **Hazards** (`res://resources/hazards/`): in Sectors 2 and 3, a battle or elite can roll a hazard with the map (15% / 25%, on the "hazards" stream), shown as a colored badge on the node and named in its tooltip. Both mechs fight under it (a copy is added to each side's relics for that fight): **Ion Storm** (shields start drained), **Scorched Ground** (both start at 30 heat), **Magnetic Interference** (weapons fire 15% slower).
- **Events:** each Event node draws the next event the run can have from its pool (shuffled once, then refilled when used up; an event that can't happen yet is kept, dropped, sent to the back, or put back at random, by its own strategy; when none can, the fallback shows). An event is a story with choices; a choice can need something (gold, a mounted weapon, some parts) and says so when it's off. A choice's outcome is picked by weight and applies its effects: gold, hull repair or damage (never below 1 HP), max HP, a part into the stash, losing random parts (stash first), a relic, reworking the strongest mounted weapon, a status for a few fights, or a fight (with its tier's loot). The events, from the design notes (`res://resources/events/`):
  ...
  | Scrapyard Auction (Sectors 1-2) | Bid 20 gold: a rare part (3 in 5) or an uncommon one. Bid 40: a rare part and a common relic. Walk away. |
  | The Pit (Sectors 2-3) | Step in: fight the Pit Champion (an elite Striker with a Leech Drill, Shock Lance, and Rivet Mortar) for a rare relic on top of its loot. Bet 15 gold on the champion: even odds to win 40. Leave. |
  | A Sealed Crate | Haul it aboard: a 2×2 junk crate, unsellable, that cracks open into a random rare part (into the stash) after 3 won fights installed; the next loot screen says so. Leave it. |
  | Reactor Leak (Sector 2) | Siphon the core (the Reactor only): +100 max HP. Seal it: 30 hull damage, +30 gold. Leave. |
  | Foundry Recast (Sectors 2-3) | Feed the furnace (needs a stashed part): a random stashed part becomes a random part of the next rarity up (the same rarity when there's none). Leave. |
  | Pilot's Shrine | Leave your junk (needs junk): every junk part goes, 15 gold each. Rest a moment: repair 40. Move on. |
  | Salvager Ambush | Hand over a random part (stash first), or fight them off (a normal battle). |
  | Mod Vendor (needs a mounted weapon) | 15 gold each: Rapid, Stabilized, or Siphon on the strongest weapon. Move on. |
  | A Derelict Carrier (Sectors 1-2, two pages) | Board it (sets the run's `carrier_boarded` flag): the hangar page offers two random field kits, or forcing the reactor room (an uncommon relic, or an elite fight), or getting out. Leave it. |
  | The Carrier's Black Box (Sectors 2-3, after boarding the carrier) | Decode it: a rare relic. Sell the coordinates: +50 gold. |
  | Data Cache | Download the targeting data: a Targeting Computer. Sell the location: +35 gold. |
  | Abandoned Cache (the fallback) | +15 gold. |

  An event for part of the run carries a `SectorRequirement` and waits in the pool (KEEP) until then. Pages live in `res://resources/events/pages/`, out of the pool.
...
A sector: `id`, `sector_name`; the map's shape (`floors` 12, `columns` 7, `paths` 6, `min_special_floor` 4, and an optional `generator` script extending `MapGenerator`); node weights (`battle_weight` ... `shop_weight`, `unknown_weight`, `cache_weight`, `get_node_weights()`); hazards (`hazard_chance`, `hazards`); `enemies` of every tier (`get_enemies(tier)`); enemy scaling (`enemy_hp_scale`, `enemy_hp_per_floor`, `get_enemy_hp_scale(floor_index)`); and loot gold (`battle_gold`, `elite_gold`, `boss_gold`, each x to y; `get_gold_range(tier)`).
...
- **Story and map extras:** `flags` (story flags events set and check), `resolve_unknown(node)` (`UNKNOWN_ODDS`), `roll_cache()` (a `FightReward` with a `title`), and hazards rolled onto fights with each map. `make_player_mech(node)` and `make_enemy_mech(node)` add a copy of the node's hazard. A win counts toward each installed crate (`MechPart.opens_after_wins`, `opens_into`, `crate_wins`); a crate that's due is replaced by a random catalog part of its rarity in the stash, and the next `roll_reward` carries the news in `FightReward.notes`.
...
- **`MapNode.gd`:** `id` ("floor_column", or "boss"), `floor_index` (0 at the bottom), `column`, `type` (Enum: BATTLE, ELITE, SHOP, HANGAR, EVENT, BOSS, UNKNOWN, CACHE), `next`, `visited`, `enemy`, and `jitter` (its drawn offset from its lattice point), an elite's `affixes` (rolled with the map from the run's `affix_pool`, `elite_affixes` each), and a fight's `hazard`. `is_fight()`, `get_tier()`.
