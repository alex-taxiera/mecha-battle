...
extends Control
## Draws a chassis's slot layout small: usable slots light, locked ones (the frame can grow into
## them) outlined, cut-away ones faint, and its hardpoint bays around them in a weapon tint. The chassis select screen shows one per frame,
...
const LOCKED_COLOR := Color(0.42, 0.47, 0.55, 0.55)
...
func _draw() -> void:
	...
	for y in chassis.size.y:
		for x in chassis.size.x:
			...
			if chassis.is_locked(cell):
				_draw_cell(cell, CUT_AWAY_COLOR)
				_draw_cell(cell, LOCKED_COLOR, false)
			else:
...
func _draw_cell(cell: Vector2i, color: Color, filled := true) -> void:
	...
	if filled:
		...
		draw_rect(rect.grow(-1.0), color, false, 2.0)
