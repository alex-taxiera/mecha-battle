...
func show_stats(current: MechStats, preview: MechStats, chassis: MechChassis) -> void:
	_set_stat(hp, str(current.hp), current.hp, preview.hp if preview else current.hp, "%d from chassis" % chassis.base_hp)
